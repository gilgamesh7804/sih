...
function route(){
  window.location.href = "http://127.0.0.1:5000/culture/delhi";
}
...
let currentSlide = 0;
      ...
      function updateLeftText(index) {
        link = "#";
        if(index == 0) link = "/city/delhi";
        if(index == 1) link = "/city/rishikesh";
        if(index == 2) link = "/city/amritsar";
        if(index == 3) link = "/hiddenlist";
        leftTextSection.innerHTML = `
          <h1>${textContent[index].title}</h1>
          <p>${textContent[index].date}</p>
          <p>${textContent[index].description}</p>
          <a href=${link} class="button">KNOW MORE</a>`;
      }
